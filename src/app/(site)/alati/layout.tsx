  ...
  return (
    <div className="container mx-auto px-6 py-8 grid grid-cols-12 gap-6">
      ...
      <aside className="col-span-12 md:col-span-3">
        <nav className="rounded-xl bg-gray-100 p-3 ring-1 ring-gray-200">
          <ul className="space-y-1">
            {tools.map((t) => {
              const active = pathname === t.href || (t.href !== '/alati' && pathname?.startsWith(t.href));
              return (
                <li key={t.href}>
                  <Link
                    href={t.href}
      ...
      <div className="col-span-12 md:col-span-9">
        <div className="rounded-xl bg-gray-50 p-6 ring-1 ring-gray-200">{children}</div>
      </div>
    </div>
  );
}
